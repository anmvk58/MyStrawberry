...
if __name__ == '__main__':
    ...
    df = pd.read_excel(f"C:\\Users\\anmv\\Desktop\\filenhap.xlsx")
    list_order = []
    for order in df['mã đơn']:
        list_order.append("HD00" + str(order))

    df['OrderCode'] = list_order
    new_cols = ['OrderCode', 'tổng tiền', 'chuyển khoản', 'ship']
    final_df = df[new_cols]

    writer = pd.ExcelWriter(f"C:\\Users\\anmv\\Desktop\\{filename}_result.xlsx", engine='xlsxwriter')
